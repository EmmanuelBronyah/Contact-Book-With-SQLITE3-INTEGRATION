        ...
        match user_input:
            ...
            case "add":
                try:
                    ...
                    print(textwrap.fill(str(e), width=117), '\n')
    ...
    if number_of_contacts.isdigit():
        if int(number_of_contacts) > 0:
            for i in range(1, int(number_of_contacts) + 1):
                contact = (
                    input(
                        f"Enter contact details of contact {i} "
                        f'in the form "name,number,address,email": '
                    )
                    .lower()
                    .split(",")
                )
                ...
                contact = [
                    None if cd == "" else cd for cd in contact
                ]  # cd is assigned 'None' if empty
                try:
                    name, number, address, email = contact
                    with conn:
                        c.execute(
                            "INSERT INTO Contacts VALUES (:name, :number, :address, :email)",
                            {
                                "name": name,
                                "number": number,
                                "address": address,
                    ...
                    text = (
                        "Omitted fields should be left blank. "
...
def edit_contact(conn, c):
    ...
    """
    name = input(
        "Enter the name of the person whose contact you want to edit: "
    ).lower()
    with conn:
        ...
        if contact:
            ...
            c.execute(
                "UPDATE Contacts SET name = :new_name, number = :new_number, address = :new_address, "
                "email = :new_email "
                "WHERE name = :name",
                {
...
def view_contact(conn, c):
    ...
    with conn:
        ...
        contacts = c.fetchall()
        if contacts:
            for index, cd in enumerate(
                contacts, start=1
            ):  # cd stands for contact detail
                print(
                    f"{index}. NAME: {cd[0]}, NUMBER: {cd[1]}, ADDRESS: {cd[2]}, EMAIL: {cd[3]}"
                )
...
def search_contact(conn, c):
    ...
    """
    name = input(
        "Enter the name of the person whose contact you want to find: "
    ).lower()
    with conn:
        c.execute("SELECT * FROM Contacts WHERE name = :name", {"name": name})
        contact = c.fetchone()
...
def delete_contact(conn, c):
    ...
    with conn:
        ...
        if contacts:
            name = input(
                "Enter the name of the person whose contact you want to remove: "
            ).lower()
            c.execute("SELECT * FROM Contacts WHERE name = :name", {"name": name})
            contact = c.fetchone()
            if contact:
                c.execute(
                    "DELETE FROM Contacts WHERE name = :name", {"name": contact[0]}
                )
